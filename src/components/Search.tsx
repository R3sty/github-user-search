...
export const Search = ({ handleSetSearchData }: any) => {
    ...
    return (
        <div className="mx-6 mt-4 tablet:w-full desktop:w-full">
            <form className="rounded-xl h-16 bg-white  dark:bg-bluishGreyDm shadow-md items-center flex flex-row" onSubmit={handleSubmit} >
                ...
                <input
                    ...
                    onChange={handleChange}
                        className="ml-[9px] flex-1 truncate text-xs font-light text-bluishGrey caret-[#0079FF] outline-none placeholder:text-bluishGrey dark:bg-bluishGreyDm dark: dark:text-whiteDm desktop:text-s"
                    placeholder={"Search Github username..."}
                    type="text"
                    />
                </div>
                {showNoResults && <span className='hidden pr-[14px] text-sm font-bold text-[#F74646] desktop:block'>No results</span>}
